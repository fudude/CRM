<%@ page import="com.dude.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.dude.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	* */
	...
	function changeStage(stage,i) {
		...
		$.ajax({
			...
			success : function (data) {
				...
				/*
				* data {"success":true/false "t":{交易} 即"stage":?, "possibility":?, "editBy":?, "editTime":? }
				* */
				if (data.success){

					//改变阶段成功后，1.需要在详细信息页上局部刷新 阶段 可能性 修改人 修改时间
					$("#stage").html(data.t.stage);
					$("#possibility").html(data.t.possibility);
					$("#editBy").html(data.t.editBy);
					$("#editTime").html(data.t.editTime);

					//改变阶段成功后，2.将所有的阶段图标重新判断，重新赋予样式和颜色
					changeIcon(stage,i);
	...
	function changeIcon(stage,index1) {
		...
		}else {//如果当前阶段的可能性不为零，前7个 绿圈 绿色标记 黑圈，后两个一定是黑叉
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段可能性为零 前7个一定是黑圈，后两个一个是红×一个是黑×
		...
		<%
			...
			}else {//如果当前阶段的可能性不为0 前7个有可能是绿圈，绿色标记，黑圈，后两个一定是黑×
...
//					String possibility = pMap.get(stage);
		...
		<%
					...
					}else {//如果遍历出来的阶段可能不为0，说明是前7个阶段，有可能 绿圈，绿色标记，黑圈
